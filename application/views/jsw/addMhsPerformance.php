                            ...
                            <th>Equipment :<span class="required">*</span></th>
                                <th>
                                     <select id="Equipment" name="Equipment" placeholder="Equipment" required="" class="form-control">
                                         ...
                                         <?php $qry = "Select DISTINCT Equipment from TblMHSPerformanceEC where Type='MHS'";                                        
                             ...
                             <tr>
                              ...
                              <tr>
                                <th colspan="4" style="text-align:center">
                                    <button type="button" name="save" data-id="hello" id="saveMhsPerformance" class="btn btn-success" value="save"><i class="material-icons">save</i> Save</button>
...
<script>
   $(document).ready(function(){
     ...
     function getValues(){
           ...
           $.post('<?php echo base_url();?>mhsPerformance/getValues', { Month: $Month,year:$year,Equipment:$Equipment }, function(data){ 
          var parsedData = JSON.parse(data);
          if(parsedData.err ==1){
//           $(".error_msg").html(parsedData.errors);
//            $(".error_msg").show();
//            setTimeout(hidetab,4000);
        }else{}
        
          $("#BD").val(parsedData.bd);
          $("#PM").val(parsedData.pm);
          $("#CM").val(parsedData.cm);
           });
        }
    $("#saveMhsPerformance").click(function(){
      $("#saveMhsPerformance").html('<img src="<?php echo base_url();?>site/content/img/loading.gif" style="width:25px;height:20px;" />');
      $Month= $("#Month").val();
      ...
      $CM= $("#CM").val();
      $Remark= $("#Remarks").val();
      $.post('<?php echo base_url();?>mhsPerformance/save', { Month: $Month,monthid:$monthid,year:$year,Equipment:$Equipment,RN:$RN,BD:$BD,PM:$PM,CM:$CM,Remark:$Remark }, function(data){
          //alert(data);
                    if(data==1)
                          {                                  
                                $(".success_msg").html('<i class="material-icons">check_circle_outline</i> MHS Performance Details Added Successfully');
                                $(".success_msg").show();
                                ...
                                setTimeout(hidetab,4000);
                                  ...
                                  $(".error_msg").show();
                                  setTimeout(hidetab,4000);
                                  $("#saveMhsPerformance").html('<i class="material-icons">save</i> Save');
                          }
		});
